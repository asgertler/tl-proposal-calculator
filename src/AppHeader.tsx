  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Check if email is confirmed using user metadata
        const { data: { user: currentUser } } = await supabase.auth.getUser();
        
        if (!currentUser?.user_metadata?.email_confirmed) {
        ...
        <div className="flex items-center gap-4">
          ...
          {isAuthenticated ? (
            <div className="relative">
              <button
                className="flex items-center gap-2 px-3 py-2 rounded-full hover:bg-slate-800 transition-colors"
